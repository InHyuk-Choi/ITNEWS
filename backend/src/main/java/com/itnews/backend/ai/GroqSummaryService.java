package com.itnews.backend.ai;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import okhttp3.*;
import org.jsoup.Jsoup;
import org.jsoup.nodes.Document;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.stereotype.Service;

import java.io.IOException;
import java.util.concurrent.TimeUnit;
import java.util.stream.Collectors;

/**
 * Groq API(llama-3.1-8b-instant)로 요약 시도, 실패/토큰 소진 시 Gemini로 폴백.
 */
@Service
public class GroqSummaryService {

    private static final Logger log = LoggerFactory.getLogger(GroqSummaryService.class);

    private static final String GROQ_URL = "https://api.groq.com/openai/v1/chat/completions";
    private static final String GROQ_MODEL = "llama-3.1-8b-instant";
    private static final String GEMINI_URL =
            "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent";
    private static final MediaType JSON = MediaType.get("application/json; charset=utf-8");

    private final java.util.concurrent.Semaphore semaphore = new java.util.concurrent.Semaphore(1);

    private final String groqApiKey;
    private final String geminiApiKey;
    private final OkHttpClient httpClient;
    private final ObjectMapper objectMapper;

    public GroqSummaryService(
            @Value("${app.groq.api-key:}") String groqApiKey,
            @Value("${app.gemini.api-key:}") String geminiApiKey
    ) {
        this.groqApiKey = groqApiKey;
        this.geminiApiKey = geminiApiKey;
        this.httpClient = new OkHttpClient.Builder()
                .connectTimeout(10, TimeUnit.SECONDS)
                .readTimeout(30, TimeUnit.SECONDS)
                .build();
        this.objectMapper = new ObjectMapper();
    }

    public String summarize(String title, String url) {
        if (title == null || title.isBlank()) return null;

        String content = fetchContent(url);
        String prompt = content != null
                ? "다음 IT 뉴스 기사를 한국어로 3~5문장으로 요약해줘:\n\n제목: " + title + "\n\n본문:\n" + content
                : "다음 IT 뉴스 제목을 한국어로 2~3문장으로 설명해줘:\n" + title;

        // 1차: Groq 시도
        if (groqApiKey != null && !groqApiKey.isBlank()) {
            String result = callGroq(title, prompt);
            if (result != null) return result;
            log.info("Groq failed, falling back to Gemini for: {}", title);
        }

        // 2차: Gemini 폴백
        if (geminiApiKey != null && !geminiApiKey.isBlank()) {
            return callGemini(title, prompt);
        }

        log.debug("No API key configured, skipping summarization");
        return null;
    }

    private String callGroq(String title, String prompt) {
        try {
            semaphore.acquire();
            try {
                String body = buildGroqRequest(prompt);
                Request request = new Request.Builder()
                        .url(GROQ_URL)
                        .header("Authorization", "Bearer " + groqApiKey)
                        .post(RequestBody.create(body, JSON))
                        .build();

                for (int attempt = 0; attempt < 3; attempt++) {
                    try (Response response = httpClient.newCall(request).execute()) {
                        if (response.code() == 429) {
                            long wait = (long) Math.pow(2, attempt) * 30_000L;
                            log.warn("Groq 429 rate limit, waiting {}s (attempt {})", wait / 1000, attempt + 1);
                            Thread.sleep(wait);
                            continue;
                        }
                        if (!response.isSuccessful()) {
                            log.warn("Groq API HTTP {}: {}", response.code(), response.message());
                            return null;
                        }
                        ResponseBody rb = response.body();
                        if (rb == null) return null;
                        JsonNode root = objectMapper.readTree(rb.string());
                        JsonNode text = root.path("choices").path(0).path("message").path("content");
                        return text.isMissingNode() ? null : text.asText().trim();
                    }
                }
                log.warn("Groq failed after 3 attempts for: {}", title);
                return null;
            } finally {
                Thread.sleep(5000);
                semaphore.release();
            }
        } catch (Exception e) {
            log.warn("Groq summarization error [{}]: {}", title, e.getMessage());
            return null;
        }
    }

    private String callGemini(String title, String prompt) {
        try {
            var contents = objectMapper.createObjectNode();
            var parts = objectMapper.createArrayNode();
            var part = objectMapper.createObjectNode();
            part.put("text", prompt);
            parts.add(part);
            var contentItem = objectMapper.createObjectNode();
            contentItem.set("parts", parts);
            var contentsArr = objectMapper.createArrayNode();
            contentsArr.add(contentItem);
            contents.set("contents", contentsArr);

            String url = GEMINI_URL + "?key=" + geminiApiKey;
            Request request = new Request.Builder()
                    .url(url)
                    .post(RequestBody.create(objectMapper.writeValueAsString(contents), JSON))
                    .build();

            try (Response response = httpClient.newCall(request).execute()) {
                if (!response.isSuccessful() || response.body() == null) {
                    log.warn("Gemini API HTTP {}: {}", response.code(), response.message());
                    return null;
                }
                JsonNode root = objectMapper.readTree(response.body().string());
                JsonNode text = root.path("candidates").path(0)
                        .path("content").path("parts").path(0).path("text");
                if (text.isMissingNode()) {
                    log.warn("Unexpected Gemini response structure");
                    return null;
                }
                log.debug("Gemini fallback succeeded for: {}", title);
                return text.asText().trim();
            }
        } catch (Exception e) {
            log.warn("Gemini summarization error [{}]: {}", title, e.getMessage());
            return null;
        }
    }

    private String buildGroqRequest(String userMessage) throws Exception {
        var root = objectMapper.createObjectNode();
        root.put("model", GROQ_MODEL);
        root.put("max_tokens", 512);
        root.put("temperature", 0.3);
        var messages = objectMapper.createArrayNode();
        var system = objectMapper.createObjectNode();
        system.put("role", "system");
        system.put("content", "당신은 IT 뉴스 요약 전문가입니다. 핵심만 간결하게 한국어로 요약하세요.");
        var user = objectMapper.createObjectNode();
        user.put("role", "user");
        user.put("content", userMessage);
        messages.add(system);
        messages.add(user);
        root.set("messages", messages);
        return objectMapper.writeValueAsString(root);
    }

    private String fetchContent(String url) {
        if (url == null || url.isBlank()) return null;
        try {
            Document doc = Jsoup.connect(url)
                    .userAgent("Mozilla/5.0 (compatible; ITNewsBot/1.0)")
                    .timeout(8000)
                    .get();
            String text = doc.select("p").stream()
                    .map(e -> e.text().trim())
                    .filter(t -> t.length() > 30)
                    .collect(Collectors.joining("\n"));
            if (text.isBlank()) return null;
            return text.length() > 1500 ? text.substring(0, 1500) : text;
        } catch (Exception e) {
            log.debug("Content fetch failed for {}: {}", url, e.getMessage());
            return null;
        }
    }
}
